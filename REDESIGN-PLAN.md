...
## Питання — відповіді отримано
1. Прибирання мертвого коду + highcharts — **відкладено** («лиши, видалимо коли впевнимось
   що все працює»). Залишається як майбутній чистильний коміт.
2. Кнопка «Expand» → `ChartModal` — **зроблено** (є в дизайні). Санбьорст(4 кільця)+тримап+
   детальний список на реальних даних; коміт `a1a52a5`.

## Залишкові нотатки (не блокери)
- Гліфи монет = тикер (BTC), не символ. Можна додати мапу символів для популярних валют.
- Інтеграції перевірені лише на demo; e2e з живими API-ключами не тестувалось (валідації
  збережені без змін).
- Старий код лишається мертвим у дереві (tree-shaking прибирає з бандла); чистити пізніше.
